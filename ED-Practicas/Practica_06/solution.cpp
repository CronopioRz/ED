...
int altura(const BinTree<int> &montanya, int& res)
{
   if (montanya.empty())
        return 0;
 
    int lAlt = altura(montanya.left(), res);
 
    int rAlt = altura(montanya.right(), res);
 
    res = max(res, 1 + lAlt + rAlt);
 
    return 1 + max(lAlt, rAlt);
}
...
/*El ejercicio tiene coste n en el peor de los casos siendo n el número de nodos del arbol
siendo el mismo tanto en el caso de ser equilibrado como degenerado al recorrer todos sus nodos
*/ 
