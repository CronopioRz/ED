...
class bintree_ext: public bintree<T>{
    ...
    public:
        ...
        int frontier(std::vector<T> &v) const {
            return frontier(this->raiz,v);
        }
    ...
    static int frontier(Link r, std::vector<T> &v){
        if(r == nullptr){
            return 0;
        }
        else{
            auto iz = frontier(r->left,v);
            auto dr = frontier(r->right,v);
            if (iz == 0 && dr == 0) {
               v.push_back(r->elem);
               return 1;
            }
