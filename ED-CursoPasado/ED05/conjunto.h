...
void set<T>::insert(T e) {
    if (!contains(e)) {
        if (contador == capacidad)
            amplia();
        //---------Modificamos para ordenar con la insercion--------
        size_t pos = 0; //Donde insertaremos
        //Buscamos la posicion en la que el elem sea mayor o igual
        while(pos < contador && e < datos[pos]) 
            ++pos;
        //Movemos todos los elemento una posicion a la derecha si se inserta en el medio
        size_t posMov = contador;
        while(pos < posMov){
            datos[posMov] = datos[posMov-1];
            --posMov;
        }
        //Insertamos el elemento en la posicion que acabamos de dejar libre
        datos[pos] = e;
        ++contador;
    }
}
