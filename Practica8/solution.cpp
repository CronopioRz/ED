...
private:
  ...
  static int interval_count(const T &lower, const T &upper,const Node*node);
...
int SetTree<T>::count_interval(const T &lower, const T &upper) const {
  //Enviamos el nodo a interval_count, una funcion privada con acceso a los elementos y punteros
  return interval_count(lower,upper,this->root_node);
}
...
// ... Métodos auxiliares ...
template <typename T>
int SetTree<T>::interval_count(const T &lower, const T &upper, const Node*node){
  if(node == nullptr){
    return 0;
  }
  else if ((lower <= node->elem) && (node->elem <= upper)){
    return 1 + interval_count(lower,upper,node->left) + interval_count(lower,upper,node->right);
  }
  else if (node->elem > upper){
    return interval_count(lower,upper,node->left);
  }
  else{
    return interval_count(lower,upper,node->right);
  }
}
...
bool tratar_caso() {
  ...
  //Guardamos el intervalo [lower,upper] buscamos elementos que estén en el conjunto
  //Ej: {10,34,41,43,54} y [6,12] -> 1 ya que 10 forma parte de [6,12]
